...
export async function validateRequest(
  ...
  // Validate the body of the request if defined in the terms.
  if (terms[request.method].form && terms[request.method].form!.length > 0) {
    const requestBody = await request.formData().then(formData => {
      let object: { [key: string]: any } = {}
      formData.forEach((value, key) => (object[key] = value))
      return object
    })
    const bodyKeys = Object.keys(requestBody)
    for (const key of terms[request.method].form!) {
      if (!bodyKeys.includes(key)) {
        return {
          error: {
            message: `field '${key}' is not available in the form`,
            status: Status.BadRequest,
          },
        }
      }
